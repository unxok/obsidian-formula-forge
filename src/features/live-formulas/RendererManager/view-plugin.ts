...
import { createFormulaSyntaxHighlighting } from "./utils";
					...
					tree.iterate({
						...
						enter: (node) => {
							...
							if (
								...
								// reset so codeblocks after this will get highlighted
								codeblockStart = undefined;
								codeblockEnd = undefined;
								return;
							}
 ...
 */
export const createInlineFormulaRendererPlugin = (manager: RendererManager) => {
				...
				for (const { from, to } of view.visibleRanges)
					tree.iterate({
						...
						enter: (node) => {
							...
							const { inlineCodeSyntax } = manager.plugin.getSettings();
							if (!inlineCodeSyntax) return;
...
class InlineFormulaRendererWidget extends WidgetType {
	...
	toDOM(): HTMLElement {
		const { manager, containingFile, formula } = this;
